-- ================================================================
-- admin_apply_fighter_seed_policy_b
--
-- 목적: Policy B 초기 seed를 특정 선수 1명의 fighters.stats에 실제 적용
--       단건 적용만 허용 — 전체/division 일괄 적용 없음
--
-- 안전장치:
--   1. p_fighter_id 필수
--   2. p_confirm = 'APPLY_SEED_B' 필수 (미일치 → confirmation_required)
--   3. private.is_admin() 필수 (미충족 → admin_required)
--   4. 해당 fighter 없음 → fighter_not_found
--   5. UPDATE 대상: fighters.stats, fighters.stats_updated_at 만
--
-- Policy B 공식 (preview RPC와 동일):
--   1. slpm outlier cap  : slpm_b = LEAST(slpm, 6.22)
--   2. 5축 raw score      : admin_recompute_fighter_stats 동일 공식/가중치
--   3. fight_cap          : total_fights < 3 → per-axis ceiling 68
--   4. rank blend         : Champion=0.40/75, Top5=0.30/68, Top10=0.20/63, Top15=0.10/60
--   5. floor/ceiling      : Champion(62,98) Top5(58,94) Top10(55,90) Top15(52,87)
--                           Unranked+raw(45,78) Unranked+no_raw(45,55)
--
