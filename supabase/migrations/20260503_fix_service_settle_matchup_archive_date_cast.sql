-- ================================================================
-- Fix: service_settle_matchup auto-archive event_date 캐스팅 버그
--
-- 원인: v_event.event_date(TIMESTAMPTZ)를 ::TEXT로 캐스팅하면
--       '2026-04-18 00:00:00+00' 형식이 되는데, 이를 archive_events.event_date(DATE)에
--       삽입할 때 PostgreSQL이 timezone 포함 문자열을 date로 변환하지 못해 실패.
--       오류가 EXCEPTION WHEN OTHERS 블록에서 롤백되므로
--       archive_events UPDATE까지 함께 취소돼 status가 'upcoming'으로 남는다.
--
-- 수정: ::TEXT → ::DATE (TIMESTAMPTZ를 DATE로 직접 캐스팅, 시간/timezone 제거)
-- ================================================================
